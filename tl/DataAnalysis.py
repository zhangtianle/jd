...
root_dir_local = 'D:/contest/Loan_Forecasting_Qualification/'
...
for MONTH in MONTHES:
    ...
    # 当月贷款
    loan_per_month = pd.DataFrame(
        loan.loc[loan["month"] == MONTH]["loan_amount"].groupby([loan["uid"]]).sum()).reset_index()
    # 当月月供
    pay_per_month = pd.DataFrame(
        loan.loc[loan["month"] == MONTH]["pay_per_month"].groupby([loan["uid"]]).sum()).reset_index()
    # 历史贷款总额
    ...
    feature_order = pd.DataFrame(
        {"total_price": order.loc[order["month"] == MONTH]["real_price"].groupby([order["uid"]]).sum()}).reset_index()
