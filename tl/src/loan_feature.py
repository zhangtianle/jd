...
def get_loan_feature(MONTH, NUM, uid, loan):
    ...
    min_max_loan = pd.DataFrame(
        loan.loc[loan["month"] <= MONTH]["loan_amount"].groupby([loan["uid"], loan["month"]]).sum()).reset_index()
    ...
    min_max_pay = pd.DataFrame(
        loan.loc[loan["month"] <= MONTH]["pay_per_month"].groupby([loan["uid"], loan["month"]]).sum()).reset_index()
    ...
    # 平均贷款还款周期
    average_plannum = loan.loc[loan["month"] <= MONTH].groupby(["uid"]).agg({"plannum": "mean"}).rename(
    ...
    next_month_remain_loan = pd.DataFrame(
        loan.loc[loan["month"] <= MONTH]["next_month_remain_loan"].groupby([loan["uid"]]).sum()).reset_index()
    ...
    next_month_remain_pay = pd.DataFrame(
        loan.loc[loan["month"] <= MONTH]["next_month_remain_pay"].groupby([loan["uid"]]).sum()).reset_index()
